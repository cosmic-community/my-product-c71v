import type { Testimonial } from '@/types'
import { getMetafieldValue } from '@/lib/cosmic'

function StarRating({ rating }: { rating: number }) {
  return (
    <div className="flex gap-0.5">
      {Array.from({ length: 5 }, (_, i) => (
        <svg
          key={i}
          className={`h-4 w-4 ${i < rating ? 'text-yellow-400' : 'text-gray-200'}`}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
        </svg>
      ))}
    </div>
  )
}

export default function TestimonialCard({
  testimonial,
}: {
  testimonial: Testimonial
}) {
  const customerName =
    getMetafieldValue(testimonial.metadata?.customer_name) || testimonial.title
  const roleCompany = getMetafieldValue(testimonial.metadata?.role_company)
  const quote = getMetafieldValue(testimonial.metadata?.quote)
  const rating = testimonial.metadata?.rating ?? 5
  const photo = testimonial.metadata?.photo

  return (
    <div className="flex flex-col rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
      <StarRating rating={rating} />
      {quote && (
        <p className="mt-4 flex-1 text-gray-700 leading-relaxed">"{quote}"</p>
      )}
      <div className="mt-6 flex items-center gap-3">
        {photo?.imgix_url ? (
          <img
            src={`${photo.imgix_url}?w=96&h=96&fit=crop&auto=format,compress`}
            alt={customerName}
            width={48}
            height={48}
            className="h-12 w-12 rounded-full object-cover"
          />
        ) : (
          <div className="flex h-12 w-12 items-center justify-center rounded-full bg-brand-100 text-lg font-semibold text-brand-700">
            {customerName.charAt(0)}
          </div>
        )}
        <div>
          <p className="font-semibold text-gray-900">{customerName}</p>
          {roleCompany && (
            <p className="text-sm text-gray-500">{roleCompany}</p>
          )}
        </div>
      </div>
    </div>
  )
}